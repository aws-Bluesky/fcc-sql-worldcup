...
echo "$($PSQL "SELECT DISTINCT(t.name) FROM teams AS t LEFT JOIN games as g1 ON t.team_id = g1.winner_id LEFT JOIN games AS g2 ON t.team_id = g2.opponent_id WHERE (g1.year = 2014 OR g2.year = 2014) AND (g1.round = 'Eighth-Final' OR g2.round = 'Eighth-Final') ORDER BY 1")"
